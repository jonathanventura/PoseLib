...
std::pair<CameraPose, py::dict> estimate_relative_pose_affine_with_normal_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                                                      const std::vector<Eigen::Vector2d> &points2D_2,
                                                                      const std::vector<Eigen::Vector3d> &normals_1,
                                                                      const std::vector<Eigen::Matrix2d> &affine2D_12,
                                                                      const Camera &camera1, const Camera &camera2,
                                                                      const py::dict &opt_dict,
                                                                      const std::optional<CameraPose> &initial_pose) {
    ...
    py::gil_scoped_release release;
    RansacStats stats =
        estimate_relative_pose_affine_with_normal(points2D_1, points2D_2, normals_1, affine2D_12, camera1, camera2, opt, &pose, &inlier_mask);
    py::gil_scoped_acquire acquire;
...
std::pair<CameraPose, py::dict>
estimate_relative_pose_affine_with_normal_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                      const std::vector<Eigen::Vector2d> &points2D_2, const std::vector<Eigen::Vector3d> &normals_1, const std::vector<Eigen::Matrix2d> &affine2D_12,
                                      const py::dict &camera1_dict, const py::dict &camera2_dict, const py::dict &opt_dict,
                                      const std::optional<CameraPose> &initial_pose) {
    Camera camera1 = camera_from_dict(camera1_dict);
    Camera camera2 = camera_from_dict(camera2_dict);

    return estimate_relative_pose_affine_with_normal_wrapper(points2D_1, points2D_2, normals_1, affine2D_12, camera1, camera2, opt_dict,
                                                 initial_pose);
}

std::pair<CameraPose, py::dict> estimate_relative_pose_affine_with_two_normals_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                                                      const std::vector<Eigen::Vector2d> &points2D_2,
                                                                      const std::vector<Eigen::Vector3d> &normals_1,
                                                                      const std::vector<Eigen::Vector3d> &normals_2,
                                                                      const std::vector<Eigen::Matrix2d> &affine2D_12,
                                                                      const Camera &camera1, const Camera &camera2,
                                                                      const py::dict &opt_dict,
                                                                      const std::optional<CameraPose> &initial_pose) {
    ...
    py::gil_scoped_release release;
    RansacStats stats =
        estimate_relative_pose_affine_with_two_normals(points2D_1, points2D_2, normals_1, normals_2, affine2D_12, camera1, camera2, opt, &pose, &inlier_mask);
    py::gil_scoped_acquire acquire;
...
std::pair<CameraPose, py::dict>
estimate_relative_pose_affine_with_two_normals_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                      const std::vector<Eigen::Vector2d> &points2D_2, const std::vector<Eigen::Vector3d> &normals_1, const std::vector<Eigen::Vector3d> &normals_2, const std::vector<Eigen::Matrix2d> &affine2D_12,
                                      const py::dict &camera1_dict, const py::dict &camera2_dict, const py::dict &opt_dict,
                                      const std::optional<CameraPose> &initial_pose) {
    Camera camera1 = camera_from_dict(camera1_dict);
    Camera camera2 = camera_from_dict(camera2_dict);

    return estimate_relative_pose_affine_with_two_normals_wrapper(points2D_1, points2D_2, normals_1, normals_2, affine2D_12, camera1, camera2, opt_dict,
                                                 initial_pose);
}
...
std::pair<ImagePair, py::dict>
estimate_shared_focal_relative_pose_affine_two_normals_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                            const std::vector<Eigen::Vector2d> &points2D_2, const std::vector<Eigen::Matrix2d> &affine_12, 
    ...
    py::gil_scoped_release release;
    RansacStats stats = estimate_shared_focal_relative_pose_affine_two_normals(points2D_1, points2D_2, affine_12, normals_1, normals_2, pp, opt,
                                                            &image_pair, &inlier_mask);
    py::gil_scoped_acquire acquire;
...
std::pair<Eigen::Matrix3d, py::dict> estimate_fundamental_affine_wrapper(const std::vector<Eigen::Vector2d> &points2D_1,
                                                                  const std::vector<Eigen::Vector2d> &points2D_2,
                                                                  const std::vector<Eigen::Matrix2d> &affine2D_12,
                                                                  const py::dict &opt_dict,
                                                                  const std::optional<Eigen::Matrix3d> &initial_F) {
    ...
    py::gil_scoped_release release;
    RansacStats stats = estimate_fundamental_affine(points2D_1, points2D_2, affine2D_12, opt, &F, &inlier_mask);
    py::gil_scoped_acquire acquire;
...
void register_relative_pose(py::module &m) {
    ...
    m.def("estimate_relative_pose_affine_with_normal",
          py::overload_cast<const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Matrix2d> &, const Camera &,
                            const Camera &, const py::dict &, const std::optional<CameraPose> &>(
              &estimate_relative_pose_affine_with_normal_wrapper),
          py::arg("points2D_1"), py::arg("points2D_2"), py::arg("normals_1"), py::arg("affine2D_12"), py::arg("camera1"), py::arg("camera2"),
          py::arg("opt") = py::dict(), py::arg("initial_pose") = py::none(),
          "Relative pose estimation with non-linear refinement.");

    m.def("estimate_relative_pose_affine_with_normal",
        py::overload_cast<const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Matrix2d> &, const py::dict &,
                          const py::dict &, const py::dict &, const std::optional<CameraPose> &>(
            &estimate_relative_pose_affine_with_normal_wrapper),
        py::arg("points2D_1"), py::arg("points2D_2"), py::arg("normals_1"), py::arg("affine2D_12"), py::arg("camera1_dict"), py::arg("camera2_dict"),
        py::arg("opt") = py::dict(), py::arg("initial_pose") = py::none(),
        "Relative pose estimation with non-linear refinement.");

    m.def("estimate_relative_pose_affine_with_two_normals",
          py::overload_cast<const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Matrix2d> &, const Camera &,
                            const Camera &, const py::dict &, const std::optional<CameraPose> &>(
              &estimate_relative_pose_affine_with_two_normals_wrapper),
          py::arg("points2D_1"), py::arg("points2D_2"), py::arg("normals_1"), py::arg("normals_2"), py::arg("affine2D_12"), py::arg("camera1"), py::arg("camera2"),
          py::arg("opt") = py::dict(), py::arg("initial_pose") = py::none(),
          "Relative pose estimation with non-linear refinement.");

    m.def("estimate_relative_pose_affine_with_two_normals",
        py::overload_cast<const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector2d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Vector3d> &, const std::vector<Eigen::Matrix2d> &, const py::dict &,
                          const py::dict &, const py::dict &, const std::optional<CameraPose> &>(
            &estimate_relative_pose_affine_with_two_normals_wrapper),
        py::arg("points2D_1"), py::arg("points2D_2"), py::arg("normals_1"), py::arg("normals_2"), py::arg("affine2D_12"), py::arg("camera1_dict"), py::arg("camera2_dict"),
        py::arg("opt") = py::dict(), py::arg("initial_pose") = py::none(),
        "Relative pose estimation with non-linear refinement.");
    ...
    m.def("estimate_shared_focal_relative_pose_affine_two_normals", &estimate_shared_focal_relative_pose_affine_two_normals_wrapper, py::arg("points2D_1"),
          py::arg("points2D_2"), py::arg("affine_12"), py::arg("normals_1"), py::arg("normals_2"), py::arg("pp") = Eigen::Vector2d::Zero(), py::arg("opt") = py::dict(),
          py::arg("initial_image_pair") = py::none(),
          "Relative pose estimation with unknown equal focal lengths with non-linear refinement.");
    ...
    m.def("estimate_fundamental_affine", &estimate_fundamental_affine_wrapper, py::arg("points2D_1"), py::arg("points2D_2"), py::arg("affine2D_12"),
          py::arg("opt") = py::dict(), py::arg("initial_F") = py::none(),
          "Fundamental matrix estimation with non-linear refinement. Note: if you have known intrinsics you should use "
          "estimate_relative_pose instead!");
